...
> 只想查命令怎么敲，看 **[HANDBOOK.md](HANDBOOK.md)**（运维速查）。本文讲架构和原理。
...
会打印一条 `http://<中转1IP>:<随机端口>/<随机token>.yaml`。
Clash Verge：配置 → `+` → **Remote** → 粘贴这条 http 链接 → 保存。

> 脚本也会打印一条 `clash://install-config?url=...`。那是给系统的协议处理器用的
> （浏览器地址栏敲、或直接点），**不能粘进 Verge 的输入框**——那个框只收
> `http://` / `https://` 开头的地址，粘 `clash://` 会报「无效的订阅链接」。
>
> 订阅一旦导入过，以后就别再新建了，在那张卡片上点 **↻** 更新即可。
> 前提是更新的那一刻服务器上的订阅服务开着（见下面「链接失效 ≠ 订阅失效」）。
...
```
直出 <中转1IP>            走中转1，直接从中转1 出网
直出 <中转2IP>            走中转2，直接从中转2 出网
<中转1前两段> → <落地1出口IP>   走中转1，转给落地1
<中转2前两段> → <落地2出口IP>   走中转2，转给落地2
```
